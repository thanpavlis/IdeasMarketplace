...
// Create connection
    $conn = sqldb();
    $sql = "SELECT id,user_id,auction_id,question,date FROM questions;";
                            ...
                            <table id="com" class="table table-hover">
                                ...
                                for ($i = 0; $i < count($info); $i++) {
                                    ...
                                    if ($info[$i]["img_temp"] == 1) {
                                        ?>
                                        <td><img style="height: 100px;width: 100px;" src="../image_user.php?id=<?= $info[$i]['user_id'] ?>"></img></td>
                                        <?php
                                    } else {
                                        ?>
                                        ...
                                        echo '<td class="elipsis">' . $info[$i]["question"] . '</td>';
                                        echo '<td>' . $info[$i]["date"] . '</td>';
                                        echo '<td><a href="question_edit.php?id=' . $info[$i]['id'] . '">Επεξεργασία</a></td>';
                                        echo "</tr>";
                                        $counter++;
                                    }
                                } else {
                                    echo "<center><h1>Δεν υπάρχουν ερωτήσεις για διαχείριση !</h1></center>";
                                }
                                ?>
                        </table>
                    </div>
